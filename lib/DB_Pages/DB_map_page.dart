...
class MapPageClass {
  ...
  Future<List<String>> viewUserList(value) async {
    final snapshotUserName = await db
        .collection('User_info') // User_infoコレクションの
        .where('user_place_id',
            isEqualTo: value) // タップしたピンのuser_place_idのテーブルのみ取得
        .get();

    // もしドキュメントが見つからなかった場合は空文字列を返す
    final userData =
        snapshotUserName.docs.map((doc) => doc.data()['user_name']).toList();

    userList = List<String>.from(userData);

    final snapshotPlaceName = await db
        .collection('MapPlace') // MapPlaceコレクションの
  ...
  Future<Map<String, List<dynamic>>?> getScheduleInfo(value) async {
    ...
    final Schedule = await db
        .collection('Schedule') // Scheduleコレクションの
        .where('schedule_location',
            isEqualTo: placeName) // タップしたピンの場所を使用する予定を取得
    ...
    final groupIDHolder = Schedule.docs
        .map((doc) => doc.data()['schedule_group_id'] as int)
        .toList();

    for (int i = 0; i < groupIDHolder.length; i++) {
      final Group = await db
          .collection('Group') // MapPlaceコレクションの
          .where('group_id',
              isEqualTo: groupIDHolder[i]) // groupIDHolderと一致するグループのテーブルのみ取得
          .get();

      // グループ名を取得
      final groupName =
          Group.docs.map((doc) => doc.data()['group_name'].toString()).toList();
    ...
    // 日時を取得
    final dateTimeHolder = Schedule.docs
