...
class SearchPageClass {
  ...
  Future<String> searchPlace(String value) async {
    ...
    final snapshot = await db.collection('MapPlace')          // MapPlaceテーブルにある
  ...
  Future<List<String>> searchAttribute(String value) async {
    ...
    final AttributeId = await db.collection('Attribute')            // Attributeテーブルにある
    ...
    final UserAttributeId = await db.collection('User_info')        // User_infoテーブルにある
    .where('user_attribute_id', isEqualTo: attributeIdHolder)       // 入力された属性のユーザーが存在する場合のみ
    .limit(10)                                                      // 10個のテーブルだけ取得
    .get();                                                         // 合致したテーブルの情報を取得

    final UserAttributeName = UserAttributeId.docs.map((doc)
