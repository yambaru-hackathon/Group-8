...
import 'DB_addgroupmenber_page.dart'; // DB関数のインポート 変数の引き渡しに使用
import 'dart:math';
...
class DB_groupdetail_page_class {
  ...
  void get_id(add_user_info_id) {
    group_members_user_id = add_user_info_id;
  }

  // 絞り込み検索のread関数
  Future<void> readGroupSearch(value) async {           // valueは入力された値
  ...
  final snapshot = await db.collection('Group')       // Groupテーブルにある
    .orderBy('group_id', descending: true)            // 権限順
    .limit(1)                                         // 1取得
    .get();

    final max_group_id = snapshot.docs.isNotEmpty ? snapshot.docs.first.data()['group_id'] as int : null;


    if (group_icon != '' &&
        max_group_id != null &&
        group_members_user_id != [] &&
        group_name != '') {
